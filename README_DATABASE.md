...
### 7. Personal Intelligence (MemPod) System

#### `mempod_items`
Personal knowledge management items.

| Column | Type | Description |
|--------|------|-------------|
| `id` | SERIAL (PK) | Item identifier |
| `user_id` | VARCHAR (FK→users.id) | Item owner |
| `type` | VARCHAR | "capture", "task", "note", "goal", "knowledge" |
...
#### `goal_metrics`
Metrics for goal tracking within MemPod.

| Column | Type | Description |
|--------|------|-------------|
| `id` | SERIAL (PK) | Metric identifier |
| `goal_id` | INTEGER (FK→mempod_items.id) | Parent goal |
| `name` | VARCHAR | Metric name |
| `target_value` | INTEGER | Target value |
| `current_value` | INTEGER | Current value (default: 0) |
| `unit` | VARCHAR | Measurement unit |
| `created_at` | TIMESTAMP | Metric creation time |
| `updated_at` | TIMESTAMP | Last update time |

#### `goal_progress`
Progress entries for goal metrics.

| Column | Type | Description |
|--------|------|-------------|
| `id` | SERIAL (PK) | Progress entry identifier |
| `metric_id` | INTEGER (FK→goal_metrics.id) | Parent metric |
| `value` | INTEGER | Progress value |
| `note` | TEXT | Progress notes |
| `recorded_at` | TIMESTAMP | Entry timestamp |
...
6. **Goal Progress**: Track metrics over time with progress entries
