...
export default function MemPod() {
  ...
  const createItemMutation = useMutation({
    mutationFn: async (data: any) => {
      await apiRequest("/api/mempod", "POST", data);
    },
    onSuccess: () => {
      toast({
        title: "Item Created",
  ...
  const updateItemMutation = useMutation({
    mutationFn: async ({ id, updates }: { id: number; updates: any }) => {
      await apiRequest(`/api/mempod/${id}`, "PATCH", updates);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/mempod"] });
    },
  ...
  const GoalCard = ({ goal }: { goal: MemPodItem }) => {
    ...
    const createMetricMutation = useMutation({
      mutationFn: async (data: any) => {
        await apiRequest(`/api/goals/${goal.id}/metrics`, "POST", data);
      },
      onSuccess: () => {
    ...
    const addProgressMutation = useMutation({
      mutationFn: async ({ metricId, value, note }: any) => {
        await apiRequest(`/api/metrics/${metricId}/progress`, "POST", { value: parseInt(value), note });
      },
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: [`/api/goals/${goal.id}/metrics`] });
        queryClient.invalidateQueries({ queryKey: ["/api/mempod"] });
